...
local function CreateESP(Player)
    ...
    local Skel = {
        HeadTorso          = HasDrawing and NewLine(Settings.SkeletonThickness, 0.8),
        TorsoHip           = HasDrawing and NewLine(Settings.SkeletonThickness, 0.8),
        TorsoLeftShoulder  = HasDrawing and NewLine(Settings.SkeletonThickness, 0.8),
        LeftShoulderElbow  = HasDrawing and NewLine(Settings.SkeletonThickness, 0.8),
        LeftElbowHand      = HasDrawing and NewLine(Settings.SkeletonThickness, 0.8),
        TorsoRightShoulder = HasDrawing and NewLine(Settings.SkeletonThickness, 0.8),
        RightShoulderElbow = HasDrawing and NewLine(Settings.SkeletonThickness, 0.8),
        RightElbowHand     = HasDrawing and NewLine(Settings.SkeletonThickness, 0.8),
        HipLeftKnee        = HasDrawing and NewLine(Settings.SkeletonThickness, 0.8),
        LeftKneeFoot       = HasDrawing and NewLine(Settings.SkeletonThickness, 0.8),
        HipRightKnee       = HasDrawing and NewLine(Settings.SkeletonThickness, 0.8),
        RightKneeFoot      = HasDrawing and NewLine(Settings.SkeletonThickness, 0.8),
    }
    ESPCache[Player] = {
        BoxTop      = HasDrawing and NewLine(),
        BoxBot      = HasDrawing and NewLine(),
        BoxLeft     = HasDrawing and NewLine(),
        BoxRight    = HasDrawing and NewLine(),
        Tracer      = HasDrawing and NewLine(1, 0.8),
        Name        = HasDrawing and NewText(12),
        Distance    = HasDrawing and NewText(10),
        HpBg        = HasDrawing and NewLine(4, 1),
        HpFill      = HasDrawing and NewLine(4, 1),
        Skeleton    = Skel,
        AimLine     = HasDrawing and NewLine(2, 0.9),
        LookingText = HasDrawing and NewText(13),
        Chams       = NewChams(),
    }
    if HasDrawing and ESPCache[Player].HpBg then
        ESPCache[Player].HpBg.Color = Palette.HealthBg
    end
    if HasDrawing and ESPCache[Player].Distance then
        ESPCache[Player].Distance.Font = 1
    end
    if HasDrawing and ESPCache[Player].LookingText then
...
local function DrawAimDirection(Player, o, Col)
    if not Settings.ShowAimDir then
        if o.AimLine then o.AimLine.Visible = false end
        return
    end
    local Char = Player.Character
    if not Char then
    ...
    if (not ok or not res) and o.AimLine then o.AimLine.Visible = false end
end
...
-- ══════════ NOCLIP LOOP ══════════
-- ИСПРАВЛЕНО: CanCollide = false для ВСЕХ BasePart (включая HRP)
...
RunService.RenderStepped:Connect(function()
    ...
    for Player, o in pairs(ESPCache) do
        -- Используем if/else вместо continue для совместимости
        ...
        if shouldProcess then
            local ok, SP, OnScreen = pcall(function()
                return Camera:WorldToViewportPoint(Player.Character.HumanoidRootPart.Position)
            end)
            if not ok or not OnScreen or SP.Z <= 0 then
                shouldProcess = false
            end
        end
        ...
        if not shouldProcess then
            ...
            local BH  = 4000 / (Camera.CFrame.Position - Root.Position).Magnitude
            local BW  = 2200 / (Camera.CFrame.Position - Root.Position).Magnitude
            ...
            -- Box
            if Settings.ShowBox and HasDrawing then
                o.BoxTop.From   = V2(X, Y)
                o.BoxTop.To     = V2(X + BW, Y)
                o.BoxBot.From   = V2(X, Y + BH)
                o.BoxBot.To     = V2(X + BW, Y + BH)
                o.BoxLeft.From  = V2(X, Y)
                o.BoxLeft.To    = V2(X, Y + BH)
                o.BoxRight.From = V2(X + BW, Y)
                o.BoxRight.To   = V2(X + BW, Y + BH)
            ...
            -- Looking at you
            if Settings.ShowLookingAtYou and HasDrawing and o.LookingText then
                ...
                if okL and isLooking then
                    ...
                    o.LookingText.Position = V2(SP2.X, Y - 35)
                    o.LookingText.Visible  = true
                else
                    o.LookingText.Visible = false
                end
            else
                if o.LookingText then o.LookingText.Visible = false end
            end
        end
    end
end)

print("[ESP] Universal ESP v7 loaded | RightShift = menu")
if HasDrawing then
    print("[ESP] Drawing API: доступен")
else
    print("[ESP] Drawing API: НЕ доступен (Chams всё равно работают)")
end
print("[ESP] Features: InfiniteJump | SpeedHack | Noclip | TP | FPS Boost")
